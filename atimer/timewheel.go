// @Title  
// @Description  
// @Author  Wangwengang  2021/8/17 下午11:01
// @Update  Wangwengang  2021/8/17 下午11:01
package atimer
...
func NewTimeWheel(name string, interval int64, scales int, maxCap int) *TimeWheel {
	...
	logger.ZapLog.Info("Init timerWhell is Done! ",zap.String("name",tw.name))
	return tw
}
...
func (tw *TimeWheel) AddTimeWheel(next *TimeWheel) {
	...
	logger.ZapLog.Info("Add timerWhell["+ tw.name+ "]'s next ["+ next.name+ "] is succ!")
}
...
func (tw *TimeWheel) Run() {
	go tw.run()
	logger.ZapLog.Info("timerwheel name = "+ tw.name+ " is running...")
}
